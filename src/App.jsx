...
export default function App() {
  ...
  async function generateRota() {
    ...
    const system = `You are a professional rota-planning engine. Return ONLY raw JSON — no markdown, no explanation, no code fences.

HARD CONSTRAINTS (NEVER BREAK):
1. Never schedule anyone on approved leave or outside their availability
2. No overlapping shifts across locations
3. Minimum 11 hours rest between shifts
4. Role eligibility must match staff skill list
5. LAST RESORT staff only if absolutely no alternative

COVERAGE RULES:
CAFÉS (Oxted + Reigate) - every shift needs: ≥1 Barista + ≥1 Kitchen
PIZZA (Thu-Sat) - every pizza shift needs: ≥2 Pizza-capable staff

SHIFT REQUIREMENTS:
Oxted: Mon-Thu 3 staff 08:30-16:00 | Fri 3 staff 08:30-16:00 + 1 staff 10:00-14:00 | Sat 2+2 staff 08:00/09:00-16:00 | Sun 4 staff 09:30-14:00
Reigate: Mon-Thu 3 staff 08:30-16:00 | Fri 3 staff 08:30-16:00 + 1 staff 10:00-14:00 | Sat 4+1 staff 08:30-16:00 | Sun 5 staff 09:30-14:00
Pizza: Thu 2 staff 16:00-22:00 + 1 staff 11:00-22:00 | Fri 3 staff 16:00-22:00 | Sat 3 staff 16:00-22:00

Return this exact JSON (all 7 days, keep staff arrays concise — first name + role only):
{"weekLabel":"${weekLabel}","days":[{"dayName":"Mon","date":"2 Mar","oxted":{"hours":"08:30-16:00","staff":[{"name":"Name","role":"Kitchen"}],"flags":[]},"reigate":{"hours":"08:30-16:00","staff":[],"flags":[]},"pizza":{"hours":"","staff":[],"flags":[]}}],"flags":[],"summary":"2 sentence summary"}`;

    const user = `Generate the rota for week of ${weekLabel}.

STAFF AVAILABILITY & ROLES:
${staffLines}

LEAVE/HOLIDAY THIS WEEK (exclude these staff):
${holLines}

Instructions:
1. Summarise coverage demand for the week
2. Identify any tight staffing days
3. Build the optimal rota following all hard constraints
4. Flag any gaps, last-resort usage, or coverage risks
